...
function generateImageFilenameFromTitle(title) {
  if (!title) return '/images/blog-placeholder.jpg';
  ...
  try {
    ...
    return response.items.map(post => {
      ...
      if (post.fields.featuredImage?.fields?.file?.url) {
        featuredImage = `https:${post.fields.featuredImage.fields.file.url}`;
      } else {
        // If no Contentful image, use local image from public folder
        featuredImage = generateImageFilenameFromTitle(post.fields.title);
      }
      ...
      return {
        ...
        publishDate: post.sys.createdAt || null,
        featuredImage,
